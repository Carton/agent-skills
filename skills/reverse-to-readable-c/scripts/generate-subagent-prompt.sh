...
printf "4. **Headers & Traceability**: Add necessary C headers (e.g., #include <stdio.h>) at the top. **MANDATORY**: Add the original function address as a comment at the top of the file using the format \`@fcn.HEX_ADDR\` (e.g., \`// Original address: @fcn.00401234\`).\n"
printf "5. **Output**: Return ONLY the cleaned C code. \n"
printf "6. **New Discoveries**: If you discover new types, structs, or global variables that should be added to the global map, list them clearly AFTER the code block.\n"
